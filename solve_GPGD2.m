function [t_sum,obj_sum,location] = solve_GPGD2(M,N,F,Rb,Rm,Ym,P_F,R,P_Rb,P_Rm,P_Ym,G,P_tau0,inv_Omega,upper,max_dis,min_dis,XYZ,plt,K,location)
t_sum = [];
for i = 1:K
    [x, beta, obj] = GPGD2(M,N,P_F,R,P_Rb,P_Rm,P_Ym,G(1:M-1,:),eval("P_tau0(1:M-1,"+string(i)+")'"),inv_Omega,upper,max_dis,min_dis,XYZ,plt,eval("location.x"+string(i)));
